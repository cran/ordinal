...
%%\VignetteIndexEntry{clmm tutorial}
...
\begin{abstract}
  ...
  with the \texttt{clmm} function in package \textsf{ordinal}. Model
...
We fit this model with the \texttt{clmm} function in
package \textsf{ordinal}. Here we save the fitted \texttt{clmm} model
in the object \texttt{fm1} (short for \texttt{f}itted \texttt{m}odel
\texttt{1}) and \texttt{print} the model by simply typing its name:
<<>>=
fm1 <- clmm(rating ~ temp + contact, random=judge, data=wine)
fm1
@ 
...
<<>>=
fm2 <- clmm(rating ~ temp + contact, random=judge, data=wine,
            Hess=TRUE, nAGQ=10)
summary(fm2)
@ 
...
<<>>=
fm4 <- clm(rating ~ temp + contact, data=wine)
anova(fm4, fm2)
@ 
...
conditional modes. These quantities are included in \texttt{clmm}
...
category $j$ or above for a third quantile judge relative to a first
quantile judge is 
<<>>=
